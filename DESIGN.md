...
### Brand Mark
`BrandMark` (`src/components/brand-mark.tsx`) — the small, everywhere-else brand expression: the real product logo (`src/assets/logo.png`), not a text wordmark. Used in the sidebar header and the mobile top bar. Its artwork is light-on-transparent (drawn for the always-dark Auth Brand Panel), so outside that fixed-dark threshold it sits on a small `oklch(0.15 0.008 235)` chip so it stays legible against the light theme too — the chip is a deliberate, scoped exception to the flat/no-extra-surface norm, only ever used behind this logo mark.
